...
function show_usage(){
    echo "Usage: trivy-scan.sh (-i <image> | -f path | -r path) [-t https://trivy.example.com -k abc123] [-g graylog.example.com -p port]
...
while getopts "hi:f:r:g:p:t:k:" flag; do
    case $flag in
        h)
            show_usage
            exit 0
...
# scan
if [ "$TVSRV" == "" ]; then
    trivy --severity HIGH,CRITICAL -f json --scanners vuln $MODE $TARGET > $TMPFILE
else
    trivy --server $TVSRV --token $TVTKN --severity HIGH,CRITICAL -f json --scanners vuln $MODE $TARGET > $TMPFILE
fi
